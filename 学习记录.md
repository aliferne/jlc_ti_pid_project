...
我已经搞定了嘉立创的教程，复刻了官方案例，接下来让我们干点别的：

- [ ] 代码优化：在官方案例中，实际上大量出现了魔法数字（UI界面），以及一些重复的代码，我们可以通过宏定义、函数封装等方式进行优化。
- [ ] 硬件扩展：我们可以通过摇杆来操纵数值的变化 / 直接操控电机（不使用PID功能）
- [x] UI优化：官方案例的UI界面相对而言比较生硬（快速切换，没有视觉上的暂留），我之前在B站看到过一些PID+UI的案例，也许可以用在这里
- [ ] LOGO设计：加上自己的logo（
- [ ] UI优化：可以考虑移植LVGL库
- [ ] 语音模块：正好把之前闲置的天问Block拿出来玩，通过语音控制加速减速等

5月30号提交的：modified[app_ui]：基本实现了PID+UI的界面效果，但是CPU承受不住，有高占用导致的电机掉频和LCD卡顿等现象。这个太难绷了
对了，附上让UI变得Q弹的源代码：
...
此外我尝试通过限制选择框绘制的时刻来加速，但发现其实效果不好（不仅卡，而且顺带把显示效果给污染了）

```c
// NOTE: 初步证明了使用这种限制条件其实依然很卡
// if (ABS(current_x - target_x) < 5 && ABS(current_y - target_y) < 5) {
// }
```

哦对了，`ABS`是个宏函数： `#define ABS(x) ((x) > 0 ? (x) : -(x))`
...
初步推测应该是我给这堆按键都分到同一组导致的，TODO: 试试分到不同组
